...
export class PresenceController {
  ...
  async getHistory(
    ...
    if (from || to) {
      where.visited_at = {};
      if (from)
        (where.visited_at as Record<string, unknown>).gte = new Date(from);
      if (to)
        (where.visited_at as Record<string, unknown>).lte = new Date(
          to + "T23:59:59.999Z",
        );
    }
  ...
  async blockIp(@Body() body: { ip: string; reason?: string }) {
    const ip = await this.presenceService.blockIp(
      body.ip,
  ...
  async blockUser(@Body() body: { userId: string; reason?: string }) {
    const user = await this.presenceService.blockUser(
      body.userId,
      body.reason ?? undefined,
    );
    return { data: user };

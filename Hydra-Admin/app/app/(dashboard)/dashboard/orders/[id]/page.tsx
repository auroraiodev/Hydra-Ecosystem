...
export default function OrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadOrder = useCallback(async () => {
    try {
      const response = await ordersAPI.get(unwrappedParams.id);
      if (response) {
  ...
  }, [order, order?.id, order?.status, itemsStatusKey, loadOrder]);
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-4">
              ...
              <span
                className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-[0.2em] border ${statusColors[order.status] || ''} border-current/10 shadow-sm`}
              >
                {order.status}
              </span>
            </div>

...
const nextConfig = {
  ...
  async headers() {
    ...
    const csp = [
      ...
      `connect-src 'self' https://*.supabase.co wss://*.supabase.co https://api.scryfall.com https://hydracollect.com wss://hydracollect.com https://*.hydracollect.com wss://*.hydracollect.com ${apiOrigin} ${apiWssOrigin} ${isDev ? 'http://localhost:* ws://localhost:* http://127.0.0.1:* ws://127.0.0.1:*' : ''}`.trim(),
      "frame-src 'none'",
